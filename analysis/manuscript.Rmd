...
nutrient$Station <- revalue(nutrient$Station, c("nearshore" = "nearshore2", "Toledo" = "nearshore1"))
nutrient$Station <- factor(nutrient$Station, levels = c("nearshore1", "nearshore2", "offshore"))
...
Station = c("nearshore2", "nearshore1", "offshore")
lat = c(41.7621666, 41.703166, 41.82666)
lng = c(-83.33, -83.25433, -83.193)
Phyco = c(median.phyco$phyco[2], median.phyco$phyco[1], median.phyco$phyco[3])
...
ggmap(myMap) +
  ...
  annotate('text', x = -83.3, y = 41.795 , label = "nearshore2", size = 4) +
  annotate('text', x = -83.24, y = 41.73, label = "nearshore1", size = 4) +
...
# Subset to Toledo station (nearshore1), full community, and not 8/11
nutrient.full <- 
  nutrient %>%
    filter(Fraction == "CNA", Date != "8/11")

nutrient.full$Station <- revalue(nutrient.full$Station, 
  c( "Toledo" = "nearshore1", "nearshore" = "nearshore2", "offshore" = "offshore"))

nutrient.full$Station <- factor(nutrient.full$Station, levels = c("nearshore1", "nearshore2", "offshore"))

makebloomplot <- function(df, yvar) {
  ggplot(df, aes_string(x = "Date", y = yvar, group = "Station", linetype = "Station")) +
    geom_line(size = .8) +
    xlab("") + 
    ylab("ug/L") +
    scale_x_discrete(
      breaks = c("6/16","7/8", "8/4","9/2", "10/6"),
...
bloomvars <- c("Phycocyanin", "Chla", "ParMC")

bloomplots <- list()

# Loop through bloomvars and make a ggplot lineplot. Add x-axis to toxin plot, but not others
for (var in bloomvars) {
  nutrient.we12 <- filter(nutrient.full, Station == "nearshore1")
  
  plot <- makebloomplot(nutrient.we12, var) + ggtitle(var)
  
  ts.legend <- g_legend(plot)
...
# Create long data frame with Cyano genus relative abundance counts
erie.cyano <- 
  erie.scale %>%
    ...
    group_by(Genus) %>%
    filter(mean(genAbund) > 0.0005) %>%
    order_dates()

# GGplot
plot_cyanogenus <- function(df) {
  ggplot(df, aes(x = Date, y = genAbund, group = Station)) + 
  geom_line(size = .8) +
  ylab("Rel. Abundance") + 
...
genusplots <- list()
cyano.genera <- c("Microcystis", "Synechococcus")

for (gen in cyano.genera) {
  gen.df <- 
    erie.cyano %>%
    filter(Station == "nearshore1") %>%
    filter(Genus == gen)
  
  plot <- plot_cyanogenus(gen.df) + ggtitle(gen)
        
  genusplots[[gen]] <- plot
  
}
################## Phyco/chla/toxin density plots #####################

densplots.bloom <- list()
#nutrient.all <- nutrient.full %>% select(Chla, Phycocyanin, ParMC, Station) %>% mutate_each_(funs(log(. + .001)), bloomvars)
nutrient.newscale <- 
  nutrient.full %>% 
    mutate(Chla = ifelse(Chla > 30, 30,
                  ifelse(Chla < 30, Chla, NA))
           ) %>%
    mutate(Phycocyanin = ifelse(Phycocyanin > 30, 30,
                         ifelse(Phycocyanin < 30, Phycocyanin, NA))
           ) %>%
    mutate(ParMC = ifelse(ParMC > 2, 2,
                         ifelse(ParMC < 2, ParMC, NA))
           )
...
for (var in bloomvars) {
  ...
  # Make plot
  plot <- 
    ggplot(nutrient.newscale, aes_string(x = var, group = "Station", linetype = "Station")) + 
    geom_line(stat = "density", size = .8, alpha = .75) + 
    ggtitle(var) + 
    ylab("density") +
    xlab("ug/L") + 
    theme(
  ...
  if (var == "Phycocyanin" | var == "Chla") {
    plot <- plot + xlim(0,31)
  } else if (var == "ParMC") {
    plot <- plot + xlim(0,2.5)
  }
        
  densplots.bloom[[var]] <- plot
  
}


################# Microcystis and Synechococcus density plots #############################

densplots.cyano <- list()

for (gen in cyano.genera) {
  # Filter to genus in dans.gen
  gen.df <- erie.cyano %>% filter(Genus == gen) #%>% mutate(logAbund = log(genAbund + .001))
  
  # Make plot
  plot <- 
    ggplot(gen.df, aes(x = genAbund, group = Station, linetype = Station)) + 
    geom_line(stat = "density", size = .8, alpha = .75) + 
    ggtitle(gen) +
    ylab("density") +
    xlab("Rel. Abundance") + 
  ...
  dens.legend <- g_legend(plot)
  
  plot <- plot + theme(legend.position = "none")
        
  densplots.cyano[[gen]] <- plot
...
bloomgrobs <- llply(bloomplots, ggplotGrob)
cyanogrobs <- llply(genusplots, ggplotGrob)
dens.cyanogrobs <- llply(densplots.cyano, ggplotGrob)
dens.bloomgrobs <- llply(densplots.bloom, ggplotGrob)

plota <- rbind(do.call("rbind", c(bloomgrobs, size = "last")), do.call("rbind", c(cyanogrobs, size = "last")), size = "last")
plotb <- rbind(do.call("rbind", c(dens.bloomgrobs, size = "first")), do.call("rbind", c(dens.cyanogrobs, size = "first")), size = "first")

ggdraw() +
  draw_plot(cbind(plota, plotb, size = "last"), x = 0.01, y = 0.01, width = .97, height = .94) +
  draw_plot(dens.legend, x = .75, y = .94, width = .02, height = .05) +
  draw_plot(ts.legend, x = .25, y = .94, width = .02, height = .05) +
...
# Create a dataframe with sample metadata and sample coordinates on PC1 and PC2 for non-cyanos
...
spearman_corr_pc <- function(pc.station, rnames) {
  ...
  # PC1 tests
  we2vs12.pc1 <- cor.test(pc.station[["nearshore2"]]$PC1, pc.station[["nearshore1"]]$PC1, method = "spearman")
  we2vs4.pc1 <- cor.test(pc.station[["nearshore2"]]$PC1, pc.station[["offshore"]]$PC1, method = "spearman")
  we4vs12.pc1 <- cor.test(pc.station[["nearshore1"]]$PC1, pc.station[["offshore"]]$PC1, method = "spearman")
  
  # PC2 tests
  we2vs12.pc2 <- cor.test(pc.station[["nearshore2"]]$PC2, pc.station[["nearshore1"]]$PC2, method = "spearman")
  we2vs4.pc2 <- cor.test(pc.station[["nearshore2"]]$PC2, pc.station[["offshore"]]$PC2, method = "spearman")
  we4vs12.pc2 <- cor.test(pc.station[["nearshore1"]]$PC2, pc.station[["offshore"]]$PC2, method = "spearman")
  ...
  names(pc.p) <- c("nearshore2 vs nearshore1", "nearshore2 vs offshore", "nearshore1 vs offshore")
  ...
  names(pc.corr) <- c("nearshore2 vs nearshore1", "nearshore2 vs offshore", "nearshore1 vs offshore")
...
for (otu in cyano.otus) {
  ...
  cyano.scale$Station <- factor(cyano.scale$Station, levels = c("offshore", "nearshore1", "nearshore2"))
...
# Function to run deseq2 differential abundance analysis. Takes a phyloseq object and factor column in sample_data as arguments
...
# Add PC1 (bloom) and PC2 (seasonal) groups into phyloseq sample_data. 
# Exclude missing data point from offshore station on 9/29
which.na <- which(is.na(braypc.htrophs.sampledata$PC1))
sample_data(heterotrophs)$PC1group <- braypc.htrophs.sampledata$PC1group[-which.na]
sample_data(heterotrophs)$PC2group <- braypc.htrophs.sampledata$PC2group[-which.na]


# Now, run deseq analyses for each station seperately for both PC1 (high bloom vs low bloom) and PC2 groups (early season vs late season)
############################# WE2 ###############################
...
#   
#   highbloom.melt$Station <- factor(highbloom.melt$Station, levels = c("offshore", "nearshore1", "nearshore"))
#   
#   g1 <- 
...
for (otu in i) {
  ...
  p$Station <- factor(p$Station, levels = c("nearshore2", "nearshore1", "offshore"))
...
for (otu in i) {
  ...
  highbloom.melt$Station <- factor(highbloom.melt$Station, levels = c("offshore", "nearshore1", "nearshore2"))
...
p.chao$Station <- factor(p.chao$Station, levels = c("nearshore2", "nearshore1", "offshore"))
...
p.invsimp$Station <- factor(p.invsimp$Station, levels = c("nearshore2", "nearshore1", "offshore"))
...
# Function to make an alpha-diversity plot with a linear fit
...
WE2v12 <- subset_samples(cyanos.noscale, Station %in% c("nearshore2", "nearshore1"))
...
make_station_bray_heatmap(
  ...
  xlab = "nearshore2",
  ylab = "nearshore1",
  title = "nearshore1 vs nearshore2"
)

g1 <- g1 + theme(legend.position = "none")

WE2v4 <- subset_samples(cyanos.noscale, Station %in% c("nearshore2", "offshore") & Date != "9/29")
...
make_station_bray_heatmap(
  ...
  xlab = "Offshore",
  ylab = "nearshore2",
  title = "nearshore2 vs offshore"
)

g2 <- g2 + theme(legend.position = "none")

WE4v12 <- subset_samples(cyanos.noscale, Station %in% c("offshore", "nearshore1") & Date != "9/29")
...
```{r}
st2 <- subset_samples(erie.scale.format, Station == "nearshore2")
st4 <- subset_samples(erie.scale.format, Station == "offshore")
st12 <- subset_samples(erie.scale.format, Station == "nearshore1")
...
WE2v12 <- subset_samples(heteros.scale, Station %in% c("nearshore2", "nearshore1"))
...
WE2v4 <- subset_samples(heteros.scale, Station %in% c("nearshore2", "offshore") & Date != "9/29")
...
WE4v12 <- subset_samples(heteros.scale, Station %in% c("offshore", "nearshore1") & Date != "9/29")
