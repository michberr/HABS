...
nutplot_facet <- function(df,nutrient,title){
        ggplot(df,
                aes_string(x="Date",y=nutrient,group="Station",shape="Station",color="Station"))+
...
nutrient$Station <- factor(nutrient$Station,levels=c("WE2","WE12","WE4")) 
...
p_phyco <- nutplot_facet(nutrient,"Phycocyanin","")
p_parmc <- nutplot_facet(nutrient,"ParMC","")


# Plot phycocyanin ; note this has a special scaling
nn<-nutrient
nn$Phycocyanin[25]<-nn$Phycocyanin[25]/2
nn$Phycocyanin[46]<-nn$Phycocyanin[46]/2
p_phyco <- nutplot_unpooled(nn,"Phycocyanin","Phycocyanin")
...
othernuts<-data.frame(sample_data(moth_good))
...
# Plot Chla
p_chla <-nutplot_unpooled(othernuts,"Chla","Chlorophyll A")

# Plot temp
p_temp <- nutplot_unpooled(othernuts,"Temp","Temperature")
...
theme_set(theme_classic())
grid_arrange_shared_legend(p_chla,p_temp,p_secchi,p_turb,p_pp,p_pon)
...
ggplot(oligo.melt,aes_string(x="Date",y="count",fill="Oligotype")) +
    ...
    scale_fill_manual(values = c("#CA3543","#1F9EAE","#694B3D","#e9e9e9")) + 
    theme(axis.title.x = element_text(size=16,face="bold"),
