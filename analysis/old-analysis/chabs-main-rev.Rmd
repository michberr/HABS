...
station_colors <- c("#1f78b4", "#33a02c", "#fb9a99")
```
...
erie_scale_0001 <- prune_taxa(tax_mean > thresh*n, erie_scale)

```

## Figure 2: Bloom and Bacterial Composition 

```{r figure2, fig.height=12, fig.width=6}
...
nutrient_cyano <-
  nutrient %>%
    ...
    order_dates() %>%
    mutate(Phyco = ifelse(Phycocyanin > 50, 50, Phycocyanin)) # lower extreme values
...
make_bloomplot <- function(df, yvar, ylab) {
  ggplot(df, aes_string(x = "Date", y = yvar, group = "Station", color = "Station")) +
...
bloom_vars <- c("Phyco", "Chla", "ParMC", "CDOM", "Cyanobacteria")
bloom_plots <- list()

# Loop through bloomvars and make a ggplot. 

  for (var in bloom_vars) {
    ...
    plot <- make_bloomplot(nutrient_cyano, var, thisylab) + ggtitle(var)
    plot <- plot + theme(legend.position = "none")
    bloom_plots[[var]] <- plot
  }
...
# Grab the grobs 
grobs <- llply(bloom_plots, ggplotGrob)

# Plot A: lineplots
plot <- do.call("rbind", c(grobs, size = "last"))
...
names(physeq_subsets) <- c("full", "Cyanobacteria", "nc-bacteria")
...
# Flip axis 2
pcoas$`nc-bacteria`$vectors[ ,2] <- -pcoas$`nc-bacteria`$vectors[ ,2] 
...
pcoa_plots <- Map(
  function(x, names) {
    ...
    scale_color_manual(
      values = c("red", "#ffae19", "#4daf4a", "#1919ff", "darkorchid3")
    ) +
    geom_point(aes(color = Month), alpha = 0.6, size = 4) +
    geom_point(colour = "grey90", size = 1.5) +
    xlab(paste("PC1 ", signif(x$values$Rel_corr_eig[1]*100, 3), "%")) +
    ylab(paste("PC2 ", signif(x$values$Rel_corr_eig[2]*100, 3), "%"))
  }, pcoas, names(pcoas)
)
...
legend <- g_legend(pcoa_plots$full)
...
######################### Combine plots ###########################
ggdraw() +
  draw_plot(pcoa_plots$full, x = 0, y = 0, width = 0.28, height = 1) + 
  draw_plot(pcoa_plots$Cyanobacteria, x = 0.28, y = 0, width = 0.28, height = 1) + 
  draw_plot(pcoa_plots$`nc-bacteria`, x = 0.56, y = 0, width = 0.28, height = 1) +
  draw_plot(legend, x = 0.88, y = .3, width = 0.1, height = .35) +
  draw_plot_label(c("A", "B", "C"), c(0, 0.28, 0.56), c(1, 1, 1), size = 20) 
...
permanova_subset <- subset_samples(erie_scale, Date != "9/29")

# Calculate bray-curtis distance
bdist <- phyloseq::distance(physeq = permanova_subset, method = "bray")

# Convert sample_data to df
sampledf <- data.frame(sample_data(permanova_subset))
...
## Figure 4: Cyanobacteria community and OTU dynamics
```{r figure3, fig.height=12, fig.width=12}


############################## Cyano timeseries PC plots ##########################


# Determine which PC's exceed broken stick model
which(pcoas$Cyanobacteria$values$Rel_corr_eig > pcoas$Cyanobacteria$values$Broken_stick)

# Create new data frame with sample metadata and significant PCs
cyanos_PC_df <- data.frame(
  SampleID = rownames(pcoas$Cyanobacteria$vectors), 
  PC1 = pcoas$Cyanobacteria$vectors[ ,1], 
  PC2 = pcoas$Cyanobacteria$vectors[ ,2], 
  row.names = NULL
)
...
cyano_df <- 
    ...
    left_join(cyanos_PC_df, by = "SampleID")  %>%
    order_dates() 

# Function to create a plot of tims(x-axis) vs PC scores (y-axis)
make_pcts_plot <- function(df, pc, eigs) {
...
cyano_pc <- c("PC1", "PC2")
cyanoRel_corr_eigs <- signif(pcoas$Cyanobacteria$values$Rel_corr_eig[1:2] *100, 2)
names(cyanoRel_corr_eigs) <- cyano_pc

cyanos_pc_plots <- list()

# For loop to create PC timeseries plots
for (pc in cyano_pc) {
  cyanos_pc_plots[[pc]] <- make_pcts_plot(cyano_df, pc, cyanoRel_corr_eigs)
}
...
# Prune low abundance taxa using thresh as mean relative abundance
tax_mean <- taxa_sums(erie_scale)/nsamples(erie_scale)
eriePrune.0001 <- prune_taxa(tax_mean > thresh*n, erie_scale)
...
for (st in stations) {
  pcCyanoStation[[st]] <- make_station_braypc(cyano_df, st)
}
...
for (PC in nonCyanoPCs) {
  nonCyanoPCPlots[[PC]] <- make_pc_plot(noncyano_df, PC, eigs = nonCyanoRelCorrEigs)
}
```
...
sample_data(nonCyano)$PC1group <- noncyano_df$PC1group
sample_data(nonCyano)$PC2group <- noncyano_df$PC2group
sample_data(nonCyano)$PC3group <- noncyano_df$PC3group
...
for (station in Stations) {
  pcNonCyanoStation[[station]] <- make_station_braypc(noncyano_df, station)
}
...
nonCyanoEnvVars <- c(
  "pH", "Turbidity", "Nitrate", "SRP", "DOC", 
  "Temp", "H2O2", "CDOM", "ParMC", "Chla", "Ammonia"
)
...
for (var in cyanoEnvVars) {
  lmSimpleCyanoPC1[[var]] <- get_lm(vars = var, response = "PC1", dat = cyano_df)
}
...
for (var in cyanoEnvVars) {
  lmSimpleCyanoPC2[[var]] <- get_lm(vars = var, response = "PC2", dat = cyano_df)
}
...
for (var in nonCyanoEnvVars) {
  lmSimpleNonCyanoPC1[[var]] <- get_lm(vars = var, response = "PC1", dat = noncyano_df)
}
...
for (var in nonCyanoEnvVars) {
  lmSimpleNonCyanoPC2[[var]] <- get_lm(vars = var, response = "PC2", dat = noncyano_df)
}
...
for (var in nonCyanoEnvVars) {
  lmSimpleNonCyanoPC3[[var]] <- get_lm(vars = var, response = "PC3", dat = noncyano_df)
}
...
```{r}
# Cyanos
cyPC1 <- get_bestsub_summary(cyanoEnvVars, "PC1", dat = cyano_df)
cyPC1
cyPC1.fit <- lm(reformulate(cyanoEnvVars[cyPC1$model[-1]], "PC1"), data = cyano_df)
summary(cyPC1.fit)

cyPC2 <- get_bestsub_summary(cyanoEnvVars, "PC2", dat = cyano_df)
cyPC2
cyPC2.fit <- lm(reformulate(cyanoEnvVars[cyPC2$model[-1]], "PC2"), data = cyano_df)
summary(cyPC2.fit)

# NonCyanos
nonCyPC1 <- get_bestsub_summary(nonCyanoEnvVars, "PC1", dat = noncyano_df)
nonCyPC1
nonCyPC1.fit <- lm(reformulate(nonCyanoEnvVars[nonCyPC1$model[-1]], "PC1"), data = noncyano_df)
summary(nonCyPC1.fit)

nonCyPC2 <- get_bestsub_summary(nonCyanoEnvVars, "PC2", dat = noncyano_df)
nonCyPC2
nonCyPC2.fit <- lm(reformulate(nonCyanoEnvVars[nonCyPC2$model[-1]], "PC2"), data = noncyano_df)
summary(nonCyPC2.fit)


noncyano_dfO <- filter(noncyano_df, Station == "offshore")

nonCyPC3 <- get_bestsub_summary(nonCyanoEnvVars, "PC3", dat = noncyano_dfO)
nonCyPC3
nonCyPC3.fit <- lm(reformulate(nonCyanoEnvVars[nonCyPC3$model[-1]], "PC3"), data = noncyano_dfO)
...
# Prune low abundance taxa using thresh as mean relative abundance
tax_mean <- taxa_sums(erie_scale100)/nsamples(erie_scale100)
erie_scale.0001 <- prune_taxa(tax_mean > thresh*n, erie_scale100)
...
associatesMerge <- left_join(
  associates, 
  data.frame(tax_table(erie_scale.0001)), 
  by = c("OTU" = "Species")
)
